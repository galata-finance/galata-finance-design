...
function SelectValue({ className, ...props }: SelectPrimitive.Value.Props) {
  return (
    <SelectPrimitive.Value
      ...
      className={cn("flex flex-1 items-center text-left font-medium text-foreground", className)}
      {...props}
    />
  )
}
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        ...
        "data-[popup-open]:border-brand data-[popup-open]:ring-2 data-[popup-open]:ring-brand/20",
    ...
    >
      ...
      <SelectPrimitive.Icon
        render={
          <span className="flex size-5 items-center justify-center rounded-md text-muted-foreground transition-colors group-data-[popup-open]/select:text-brand">
            <ChevronDownIcon className="pointer-events-none size-4 transition-transform duration-fast group-data-[popup-open]/select:rotate-180" />
          </span>
        }
      />
    </SelectPrimitive.Trigger>
...
function SelectLabel({ className, ...props }: SelectPrimitive.GroupLabel.Props) {
  return (
    <SelectPrimitive.GroupLabel
      ...
      className={cn("px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wide text-muted-foreground", className)}
      {...props}
    />
  )
}
...
function SelectItem({ className, children, ...props }: SelectPrimitive.Item.Props) {
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex min-h-9 w-full cursor-default items-center gap-2 rounded-lg px-3 py-2",
        "text-sm font-medium outline-hidden select-none",
        "transition-colors duration-fast",
        "focus:bg-surface-high focus:text-foreground",
        "data-[selected]:bg-brand/10 data-[selected]:text-foreground",
    ...
    >
      ...
      <SelectPrimitive.ItemIndicator
        render={
          <span className="pointer-events-none absolute right-3 flex size-5 items-center justify-center rounded-full bg-brand/15 text-brand" />
        }
      >
